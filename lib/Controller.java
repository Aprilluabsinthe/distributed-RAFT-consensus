 ...
 * to simulate an unreliable network, all the message from each raft peer will 
 * be processed by this network, and the network decides whether to delay or
 * drop the messages.
 *
 * Controller is an RMI server executing on an input port and managing communication
 * of all raft peers.  The instance is composed in the Config.
 */
public class Controller extends UnicastRemoteObject implements MessagingLayer {
     ...
     */
    public void register(int id, RemoteControllerInterface remoteController) {
    ...
    private static class Node {
        int id;
        RemoteControllerInterface rc;
        Node(int id, RemoteControllerInterface rc) {
            this.id = id;
            this.rc = rc;
        }
    }
